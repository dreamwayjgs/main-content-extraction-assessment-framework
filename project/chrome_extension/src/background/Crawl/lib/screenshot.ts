...
function initiateCapture(tab: chrome.tabs.Tab, callback: (...args: any[]) => any) {
...
function capture(data: any, screenshots: any[], sendResponse: any, splitnotifier: any) {
  chrome.tabs.captureVisibleTab(
    chrome.windows.WINDOW_ID_CURRENT, { format: 'png' }, function (dataURI) {
      if (dataURI) {
        var image = new Image();
        image.onload = function () {
          ...
          if (!screenshots.length) {
            ...
            if (screenshots.length > 1) {
              ...
              document.querySelector('screenshot-count')!.textContent = screenshots.length.toString();
            }
          }
          ...
          ).forEach(function (screenshot: any) {
...
function _filterScreenshots(imgLeft: number, imgTop: number, imgWidth: number, imgHeight: number, screenshots: any) {
  // Filter down the screenshots to ones that match the location
  // of the given image.
  //
  var imgRight = imgLeft + imgWidth,
    imgBottom = imgTop + imgHeight;
  return screenshots.filter(function (screenshot: any) {
...
function getBlobs(screenshots: any[]) {
...
function saveBlob(blob: Blob, filename: string, index: number, callback: any, errback: any) {
...
function _addFilenameSuffix(filename: string, index: number) {
  if (!index) {
    return filename;
  }
...
export function captureToBlobs(tab: chrome.tabs.Tab, callback: any, errback: any, progress: any, splitnotifier: any) {
  let loaded = false
  let screenshots: any[] = [],
    timeout = 3000,
    timedOut = false,
...
export function captureToFiles(tab: chrome.tabs.Tab, filename: string, callback: any, errback: any, progress: any, splitnotifier: any) {
  captureToBlobs(tab, function (blobs: Blob[]) {
    var i = 0,
      len = blobs.length,
      filenames: string[] = [];

    (function doNext() {
      saveBlob(blobs[i], filename, i, function (filename: string) {
        i++;
